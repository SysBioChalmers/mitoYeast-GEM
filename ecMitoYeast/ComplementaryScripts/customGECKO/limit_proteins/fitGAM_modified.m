function GAM = fitGAM_modified(model,verbose)
% fitGAM
%
...
if isfield(parameters,'GAM')
    ...
    %Load chemostat data:
    fid = fopen('../../../../GECKO/databases/chemostatData.tsv','r');
    exp_data = textscan(fid,'%f32 %f32 %f32 %f32','Delimiter','\t','HeaderLines',1);
    exp_data = [double(exp_data{1}) double(exp_data{2}) double(exp_data{3}) double(exp_data{4})];
    fclose(fid);
    ...
    %2nd iteration:
    GAM = iteration(model,GAM-2:0.5:GAM+2,exp_data,verbose);
    
    %3rd iteration:
    %[GAM,error] = iteration(model,GAM-1:0.25:GAM+1,exp_data,verbose);
...
model = scaleBioMass_modified(model,Pbase,GAM,false);
cd ..
parameters = getModelParameters_modified;
%cd limit_proteins
%Relevant positions:
...
for i = 1:length(exp_data(:,1))
    ...
    % Account for growth rate in constraints
    cd ../simulation/
    model_constrained = incorporateMuInConstraints(model,exp_data(i,1));
    sol   = optimizeCbModel(model_constrained,'min','one');
