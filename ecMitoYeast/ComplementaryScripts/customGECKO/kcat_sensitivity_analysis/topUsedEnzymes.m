...
%   Carl Malina       Last edited 2020-04-28 - Adapt to ME-like concept
...
usages = find(~cellfun(@isempty,strfind(model.rxns,'draw_prot_')));
%Exclude protein pool exchange reaction - not necessary since 'draw_prot'
% used to identify protein exchange rxns
...
mu = fluxes(strcmpi(model.rxnNames,'growth'));
for i=1:length(usages(1,:))
    %Units conversion [mmol/gDW] -> [g prot/gDW]
    enzUsage = (usages(:,i).*model.MWs)./mu;
...
if writeFile
    writetable(T,['../../../ModelFiles/' name '_topUsedEnzymes.txt'])
end
end