%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% model = manualModifications_modified(model)
% 
% Benjamin J. Sanchez. Last edited: 2017-10-29
% Ivan Domenzain.      Last edited: 2018-05-28
% Carl Malina.         Last edited: 2020-04-13 - Adapt to work for
% ecMitoYeast-GEM
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [model,modifications] = manualModifications_modified(model)
current = pwd;
%Read manual data:
cd ../../../../../GECKO/databases/
...
for i = 1:length(model.rxns)
    ...
    if contains(reaction,'translocation') || contains(reaction,'modification')
        continue
    elseif ~contains(reaction,'No')
        continue
    end
    %Find set of proteins present in rxn:
    S        = model.S;
    subs_pos = find(S(:,i) < 0);
    prot_pos = find(~cellfun(@isempty,strfind(model.mets,'holo_prot_')));
    int_pos  = intersect(subs_pos,prot_pos);
    prot_set = cell(size(int_pos));
    % Set compartment of enzyme
    comp_set = model.comps(model.metComps(int_pos));
    MW_set   = 0;
    for j = 1:length(int_pos)
        met_name    = model.mets{int_pos(j)};
        met_name_split = strsplit(met_name,'_');
        prot = met_name_split{3};
        prot_set{j} = prot;
    ...
    for j = 1:length(uniprots)
        ...
        if length(int)/max(length(prot_set),length(uniprots{j})) > 0.50 % 50% match
            ...
            % Update stoichiometry of enzymes
            stoichCoeffs = zeros(size(prot_set));
            stoichiometry = stoich{j};
            for k = 1:length(prot_set)
                prot = prot_set{k};
                protInUniprots = strcmp(uniprots{j},prot);
                if sum(protInUniprots) == 1
                    stoichCoeffs(k) = stoichiometry(protInUniprots);
                else
                    stoichCoeffs(k) = max(stoichiometry);
                end
            end

            %Add new protein stoich. coeffs to rxn:
            newMets = strcat('holo_prot_',prot_set)'; % Keep enzymes the same as in original reaction
            kvalues = kcats(j)./stoichCoeffs;
            for k = 1:length(prot_set)
                modifications{1} = [modifications{1}; prot_set{k}];
                modifications{2} = [modifications{2}; reaction];
            end
            rxnID   = model.rxns{i};
            %newMets{k} = ['holo_prot_' newMets{k}]; 
            rxnName = model.rxnNames{i};
            grRule  = model.grRules{i}; % Keep the original grRule, not the one defined in manual_data.txt
            model   = addEnzymesToRxn(model,kvalues',rxnID,newMets,comp_set,{rxnID,rxnName},grRule);
...
for i = 1:length(model.enzymes)
    pos_met = strcmp(model.mets,['holo_prot_' model.enzymes{i}]);
...
function [newValue,modifications] = curation_growthLimiting(reaction,enzName,MW_set,modifications)  
        ...
        % Changed manually (2015-08-28).
          if contains('holo_prot_P06168_m',enzName) 
        ...
        % https://www.ncbi.nlm.nih.gov/pmc/articles/PMC177616/
          if (strcmpi('holo_prot_P12683',enzName)||strcmpi('holo_prot_P12684',enzName)) && ...
        ...
        % value seems to be the best candidate for this reaction (rev)
         if strcmpi('holo_prot_P06169',enzName) && contains(reaction,'pyruvate decarboxylase')
        ...
        % umol/min/mg) from BRENDA (2018-01-22).
          if strcmpi('holo_prot_Q00955',enzName) && contains(reaction,'acetyl-CoA carboxylase')
        ...
        % from BRENDA (2018-04-10).
          if strcmpi('holo_prot_P52867',enzName) && contains(reaction,'dolichyl-phosphate-mannose--protein mannosyltransferase')
        ...
        % from BRENDA (2018-04-10).
          if strcmpi('holo_prot_P10659',enzName) && contains(reaction,'methionine adenosyltransferase')
...
function [newValue,modifications] = curation_carbonSources(reaction,enzName,MW_set,modifications)
       ...
       % from BRENDA (2018-01-22).
         if (strcmpi('holo_prot_P32356',enzName)  || strcmpi('prot_P48016',enzName)) && ...
       ...
       % from BRENDA (2018-01-22).
         if strcmpi('holo_prot_P00331',enzName) && contains(reaction,'alcohol dehydrogenase (ethanol to acetaldehyde)')
             newValue      = -(143*3600)^-1;
        ...
        % is 57 kDa. From BRENDA (2018-01-26).
         if strcmpi('holo_prot_P14065',enzName) && contains(reaction,'glycerol dehydrogenase (NADP-dependent)')
...
function [newValue,modifications] = curation_topUsedEnz(reaction,enzName,MW_set,modifications)
        ...
        reaction = string(reaction);
          ...
          % instead)
          if strcmpi('holo_prot_P04046',enzName) && contains(reaction,'phosphoribosylpyrophosphate amidotransferase')
          ...
          if contains('holo_prot_Q04728_m',enzName) && contains(reaction,'ornithine transacetylase (No1)')
...
%           if (strcmpi('holo_prot_P07251_mm',enzName) || strcmpi('holo_prot_P00830_mm',enzName))&& ...
...
%           end
          ...
          % from BRENDA (2017-01-18).
          if strcmpi('holo_prot_P15019',enzName) && contains(reaction,'transaldolase (reversible)')
          ...
          % from BRENDA (2018-01-22).
          if strcmpi('holo_prot_Q06817',enzName) && contains(reaction,'glycyl-tRNA synthetase')
          ...
          if strcmpi('holo_prot_P80235_mm',enzName) && contains(reaction,'carnitine O-acetyltransferase')
        ...
        % (2015-08-25)
        if (strcmpi('holo_prot_P07149',enzName) || strcmpi('holo_prot_P19097',enzName))
        ...
        % both measurements are for native enzymes
          if strcmpi('holo_prot_P00924',enzName)  && contains(reaction,'enolase')
               newValue      = -1/(230*3600);
...
function model = otherChanges(model)
    ...
    % (2017-01-16):
     ...
     for i = 1:length(rxns_tochange)
         ...
         model.S(strcmpi(model.mets,'holo_prot_P14540'),pos_rxn) = 0;
    ...
    %(2015-11-05)
    index = find(strcmpi('holo_prot_P19414_m',model.mets));
    if ~isempty(index)
    ...
    % Succinate-semialdehyde dehydrogenase (P38067/EC1.2.1.16): The retrieved kcat 
    % value from BRENDA was from E. coli under extreme conditions. The value was 
    % corrected by using the specific activity for S. cerevisiae in BRENDA, 
    % equal to 0.66 µmol/min/mg (reaction r_1023).
    index = find(strcmpi('holo_prot_P38067',model.mets));
    rxnInd = find(strcmp('r_1023No1',model.rxns));
    MW     = model.MWs(strcmp(model.enzymes,'P38067'));
    newValue = -1/(0.66*60*MW);
    model.S(index,rxnInd) = newValue;
    % 2-oxoglutarate dehydrogenase (P20967-P19262-P09624/1.2.4.2; 1.8.1.4; 2.3.1.61)
    % is represented as a two-step reaction so the kcats should be multiplied by two
    metInd = find(contains(model.mets,{'holo_prot_P20967_m','holo_prot_P19262_m','holo_prot_P09624_m'}));
    rxnPos = find(contains(model.rxns,{'r_0831No1','r_0832No1'}));
    for i = 1:length(rxnPos)
        model.S(metInd,rxnPos(i)) = model.S(metInd,rxnPos(i))/2;
    end
end
